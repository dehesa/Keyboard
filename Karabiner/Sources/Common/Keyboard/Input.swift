...
public extension Keyboard {
    /// The input state received from the keyboard.
    ///
    ...
    public struct Input: Codable {
        ...
        /// List of mandatory modifier to be applied.
        public let mandatory: Modifiers
        /// List of optional modifier to be applied
        public let optional: Modifiers
        
        /// Designated initializer.
        public init(_ kind: Kind, _ mandatory: Modifiers = .none, optional: Modifiers = .none) {
            self.kind = kind
        ...
        public init(from decoder: Decoder) throws {
            ...
            if let keyCode = try container.decodeIfPresent(Keyboard.Key.self, forKey: .keyCode) {
                ...
                kind = .custom(customCode)
            } else if let either = try container.decodeIfPresent(Kind.`Any`.self, forKey: .any) {
                kind = .any(either)
            } else {
                let context = DecodingError.Context(codingPath: container.codingPath, debugDescription: "It was impossible to figure out what input is expected (e.g. whether keyCode, pointing button, customer code, etc.).")
                throw DecodingError.dataCorrupted(context)
            }
            
            guard container.contains(.modifiers) else {
                self.init(kind, .none, optional: .none); return
            }
            ...
            self.init(kind, mandatory, optional: optional)
        }
        ...
        public func encode(to encoder: Encoder) throws {
            ...
            if case .none = self.mandatory, case .none = self.optional { return }
            var nestedContainer = container.nestedContainer(keyedBy: CodingKeys.self, forKey: .modifiers)
            
            switch self.mandatory {
            case .none: break
            default: try nestedContainer.encode(self.mandatory, forKey: .mandatory)
            }
            
            switch self.optional {
            case .none: break
            default: try nestedContainer.encode(self.optional, forKey: .optional)
            }
        }
...
public extension Keyboard.Input {
    ...
    public enum Kind {
        ...
        case any(Keyboard.Input.Kind.`Any`)
